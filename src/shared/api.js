...
export class DdysExtensionApi {
  ...
  async request(path, query = {}, options = {}) {
    ...
    const timeout = setTimeout(() => controller.abort(new Error('DDYS API request timeout.')), this.settings.requestTimeoutMs);
    let response;
    let json;
    try {
      ...
      throw new Error(error?.message || 'DDYS API request failed.');
    } finally {
    ...
    if (json?.success !== true && !Array.isArray(json) && typeof json !== 'object') {
      throw new Error('DDYS API response is invalid.');
    }
  ...
  async hot() {
    return unwrapData(await this.request('/hot'));
  }
